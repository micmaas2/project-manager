...
class DailyFactsAgent(BaseAgent):
    ...
    async def _generate_fact_with_llm(self, category: Optional[str] = None, force_new: bool = False) -> Dict[str, Any]:
        ...
        # Build LLM prompt with excluded persons
        prompt = self._build_fact_generation_prompt(category, min_words, max_words, excluded_persons=excluded_persons)
    ...
    def _build_fact_generation_prompt(
        ...
        category_guidance = {
            "born_today_quote": f"""LANGUAGE: DUTCH
Find a notable scientist or philosopher born on {month_day} (any year).
Share one of their most meaningful or thought-provoking quotes WITH context.
Include: Name, birth/death years, brief bio, why the quote matters.""",
            "born_today_discovery": f"""LANGUAGE: DUTCH
Find a notable scientist or philosopher born on {month_day} (any year).
Focus on their major discovery, achievement, or contribution to science/philosophy.
Include: Name, birth/death years, what they discovered, why it's significant.""",
        }

        guidance = category_guidance.get(category, category_guidance["born_today_quote"])
