...
| BL-107 | EPIC-007 | CCAS: SAP credential PreToolUse blocking hook — block Write/Edit when SAP credential patterns detected at edit time; emit file path + line number only (never matched text); mirrors BL-101 security-guidance pattern; prevents credentials from ever entering git history | CCAS | P2 | in_progress | 2026-04-28 |
| BL-108 | EPIC-007 | pi-homelab: HA YAML syntax + Lovelace structure pre-commit hook — validate Home Assistant YAML and Lovelace config before push; prevents failed HA restarts from config errors | pi-homelab | P2 | in_progress | 2026-04-28 |
...
| BL-110 | EPIC-007 | pensieve: n8n JSON workflow syntax pre-commit hook — validate telegram-capture.json and gmail-capture.json syntax before push; prevents corruption of live capture workflows | pensieve | P2 | in_progress | 2026-04-28 |
...
| BL-112 | EPIC-007 | genealogie: Python lint (ruff/black) pre-commit hook — enforce code style as Python genealogy codebase grows; prevent style drift | genealogie | P2 | in_progress | 2026-04-28 |
| BL-113 | EPIC-007 | genealogie: SQLite schema validator pre-commit hook — validate db.py migrations before push; protect genealogy data integrity across schema changes | genealogie | P2 | in_progress | 2026-04-28 |
...
| BL-118 | — | project_manager: CLAUDE.md size reduction pass — file is 41.7 KB vs ≤35 k target from task-037; 6.7 KB of net growth from session learnings; migrate verbose sections (MVP template security checklist, shell pre-submission list) to linked docs; target ≤38 k | project_manager | P3 | new | 2026-04-30 |
| BL-119 | — | project_manager: MAS orchestration efficiency audit — analyse current pm-run pipeline for bottlenecks; evaluate: (1) subagent parallelism beyond current Reviewer+CQR and DocUpdater+docs-readme-writer pairs, (2) dedicated YAML agents vs built-in subagent_types for each pipeline stage, (3) skill extraction — which pm-* skills could be replaced by lighter YAML agents or remote triggers, (4) context window usage per stage — where is main context polluted unnecessarily; deliverable: ranked list of efficiency proposals with estimated token/latency savings | project_manager | P2 | new | 2026-04-30 |
