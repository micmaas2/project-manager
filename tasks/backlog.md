...
| BL-106 | — | CCAS: Ansible playbook syntax + idempotency pre-commit hook — enforce ansible-lint and validate roles are idempotent before push; linters exist but are not currently hooked | CCAS | P2 | new | 2026-04-28 |
| BL-107 | — | CCAS: SAP credential PreToolUse blocking hook — block Write/Edit when SAP credential patterns detected at edit time; emit file path + line number only (never matched text); mirrors BL-101 security-guidance pattern; prevents credentials from ever entering git history | CCAS | P2 | new | 2026-04-28 |
| BL-108 | — | pi-homelab: HA YAML syntax + Lovelace structure pre-commit hook — validate Home Assistant YAML and Lovelace config before push; prevents failed HA restarts from config errors | pi-homelab | P2 | new | 2026-04-28 |
...
| BL-110 | — | pensieve: n8n JSON workflow syntax pre-commit hook — validate telegram-capture.json and gmail-capture.json syntax before push; prevents corruption of live capture workflows | pensieve | P2 | new | 2026-04-28 |
...
| BL-112 | — | genealogie: Python lint (ruff/black) pre-commit hook — enforce code style as Python genealogy codebase grows; prevent style drift | genealogie | P2 | new | 2026-04-28 |
| BL-113 | — | genealogie: SQLite schema validator pre-commit hook — validate db.py migrations before push; protect genealogy data integrity across schema changes | genealogie | P2 | new | 2026-04-28 |
| BL-114 | — | school-ai: new project — AI tutoring assistant for primary school kids (werkstuk/assignments); guides how to work and learn without writing for the child; scaffolding-only approach | school-ai | P2 | new | 2026-04-28 |
